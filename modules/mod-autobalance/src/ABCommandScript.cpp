...
bool AutoBalance_CommandScript::HandleABMapStatsCommand(ChatHandler* handler, const char* /*args*/)
{
    ...
    if (player->GetMap()->IsDungeon())
    {
        ...
        int8 const effectiveOffset =
            GetEffectivePlayerCountDifficultyOffset(mapABInfo->playerCount);
        ...
        else if (mapABInfo->playerCount < mapABInfo->minPlayers && !effectiveOffset)
            handler->PSendSysMessage(ABGetLocaleText(locale, "adjusted_player_count_map_minimum").c_str(), mapABInfo->adjustedPlayerCount);
        else if (mapABInfo->playerCount < mapABInfo->minPlayers && effectiveOffset)
            handler->PSendSysMessage(ABGetLocaleText(locale, "adjusted_player_count_map_minimum_difficulty_offset").c_str(), mapABInfo->adjustedPlayerCount, effectiveOffset);
        else if (effectiveOffset)
            handler->PSendSysMessage(ABGetLocaleText(locale, "adjusted_player_count_difficulty_offset").c_str(), mapABInfo->adjustedPlayerCount, effectiveOffset);
        else
