...
void UpdateMapPlayerStats(Map* map)
{
    ...
    //
    // Adjust by the PlayerDifficultyOffset (optional cap by actual party size)
    //

    adjustedPlayerCount += static_cast<uint8>(
        GetEffectivePlayerCountDifficultyOffset(mapABInfo->playerCount));

    //
    // Never scale as if more players than the instance allows (e.g. offset +1
    // with a full 5-player party must not become 6 for a 5-man).
    //
    uint32 const maxPlayers = instanceMap->GetMaxPlayers();
    if (maxPlayers > 0 && static_cast<uint32>(adjustedPlayerCount) > maxPlayers)
        adjustedPlayerCount = static_cast<uint8>(maxPlayers);
